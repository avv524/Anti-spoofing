...
class EERMetric(BaseMetric):
    """
    Equal Error Rate (EER) metric for voice anti-spoofing evaluation.
    """
    ...
    def update(self, logits, labels, **batch):
        ...
        """
        # Higher score = more bonafide, lower score = more spoof
        if logits.dim() == 2 and logits.size(1) == 2:
            log_probs = torch.log_softmax(logits, dim=1)
            # Since labels are 1=bonafide, 0=spoof, we need to map logits accordingly
            # logits[:, 1] should correspond to bonafide (label=1)
            # logits[:, 0] should correspond to spoof (label=0)
            scores = log_probs[:, 1] - log_probs[:, 0]  # log(P(bonafide)) - log(P(spoof))
        else:
            scores = logits.squeeze()
        
        # Store scores and labels
        self.scores.extend(scores.cpu().detach().numpy())
        self.labels.extend(labels.cpu().detach().numpy())
    
    def compute(self):
        """
        Compute EER from accumulated scores and labels using official implementation
        
        Returns:
            float: EER value as percentage
        """
        ...
        # Split scores by labels: bonafide (1) vs spoof (0)
        bonafide_scores = scores[labels == 1]
        spoof_scores = scores[labels == 0]
        
        if len(bonafide_scores) == 0 or len(spoof_scores) == 0:
            return 50.0  # Fallback EER
    ...
    def __call__(self, logits, labels, **batch):
        """
        Compute EER for current batch and update running average
        
        Returns:
            float: current EER value
        """
        self.update(logits, labels, **batch)
        return self.compute()